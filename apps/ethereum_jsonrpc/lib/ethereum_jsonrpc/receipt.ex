defmodule EthereumJSONRPC.Receipt do
  ...
  defp entry_to_elixir({key, _} = entry)
       when key in ~w(blockHash contractAddress from gas logsBloom root to transactionHash revertReason type effectiveGasPrice statusStr outData),
       do: {:ok, entry}

  defp entry_to_elixir({key, quantity})
       when key in ~w(blockNumber cumulativeGasUsed gasUsed transactionIndex blobGasUsed blobGasPrice statusStr outData) do
    result =
  ...
  defp entry_to_elixir({key, value}) do
    {:error, {:unknown_key11, %{key: key, value: value}}}
  end
end
